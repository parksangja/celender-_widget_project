...
class CalendarEngine:                               #엔진 클래스 정의
        ...
        for target_date in self._iter_candidate_dates(start.date(), recurrence, recurrence_end): #반복 설정에 따라 발생할 수 있는 날짜들을 순회하면서 충돌 검사
            ...
            for event in self.events:
                ...
                if ignore_id is not None and event["id"] == ignore_id:  #업데이트 시 자기 자신과의 충돌은 무시하도록 설정되어 있으면 패스
                    continue
                if not self._timed_occurs_on(event, target_date):       #이벤트가 목표 날짜에 발생하지 않으면 패스
                    continue

                existing_start, existing_end = self._timed_bounds_on(event, target_date)    #이벤트와 목표 날짜를 받아 시작과 끝 계산
    ...
    def _to_dict(self, event, target_date=None):            #이벤트를 딕셔너리로 반환하는 함수
    ...
    def _save(self):                                                #이벤트 저장 함수
        ...
        with open(self.storage_path, "w", encoding="utf-8") as f:   #저장경로를 열어 위 데이터들을 덮어쓰고 저장
            json.dump(data, f, ensure_ascii=False, indent=2)        #ensure_ascii=False로 유나코드 변환 없이 그대로 저장

    def _load(self):
        if not os.path.exists(self.storage_path):                 #저장경로가 없으면 pass
            return

        with open(self.storage_path, "r", encoding="utf-8") as f: #저장경로로 events.json이 있으면 파일을 읽기모드로 열음
            data = json.load(f)                                   #파일 데이터를 읽어 data에 저장
    ...
    def list_events(self, date=None):
        ...
        for event in self.events:
            ...
            if self._timed_occurs_on(event, target_date):           #기간 이벤트가 아닌 이벤트에 대해 검사,
                result.append(self._to_dict(event, target_date))    #타겟 날짜와 같으면 result에 이벤트 추가
