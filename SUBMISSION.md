...
현재는 PyQt6 UI, 일정 엔진, OpenAI API 입력 해석, 로컬 한국어 자연어 파서, 명령 실행기, 직접 일정 추가/수정 기능, 반복 일정, 공휴일 자동 업데이트 기능까지 구현되어 있습니다.
...
- `main.py`: PyQt6 기반 UI 실행 파일입니다. 3분할 화면, AI 입력창, 캘린더, 일정 목록을 담당합니다.
- `ui_support.py`: 설정 창, 직접 추가 창, 일정 수정 창, 색상 선택, AI 확인 문구 처리 같은 UI 보조 기능을 담당합니다.
- `ui_styles.py`: 위젯 전체 스타일시트를 담당합니다.
